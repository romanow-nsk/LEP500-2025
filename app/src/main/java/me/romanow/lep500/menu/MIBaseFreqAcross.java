...
public class MIBaseFreqAcross extends MenuItem{
    public MIBaseFreqAcross(MainActivity main0) {
        super(main0);
        main.addMenuList(new MenuItemAction("Базовая частота поперечная") {
            @Override
            public void onSelect() {
                main.selectMultiFromArchive("Поперечные измерения",procViewMultiSelector);
            }
        });
    }
    //------------------------------------------------------------------------------------
    private I_ArchiveMultiSelector procViewMultiSelector = new I_ArchiveMultiSelector() {
        ...
        public void onSelect(FileDescriptionList fd, boolean longClick) {
            ...
            main.defferedFinishBaseFreq(true);
            }
    };
}
